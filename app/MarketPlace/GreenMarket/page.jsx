// app/marketplace/greenmarket/page.jsx
"use client";
  ...
  return (
    <div className="container mx-auto p-6 mt-7 mb-[150px]">
      <h1 className="text-3xl text-center font-semibold mb-6">
        Empower your choices—buy, rent, and share for a sustainable tomorrow.
      </h1>
      ...
      {items.length === 0 ? (
        ...
        <ItemGrid>
          {items.map(item => (
            <a key={item.id} href={`/MarketPlace/GreenMarket/${item.id}`}>
              <Card>
                <div className="card-img">
                  {/* Display the image if it exists, otherwise show the placeholder */}
                  {item.images ? (
                    <img
                      src={item.images[0]} // Assuming 'item.images' is an array
                      alt={item.title}
                      style={{
                        width: '100%',
                        height: '100%',
                  ...
                  {/* Wrapper for the labels to position them correctly */}
                  <LabelWrapper>
                ...
                <div className="card-footer">
                  <span className="text-title">
                    {/* Conditionally render the price based on the item type */}
...
// Placeholder component for items with no image
...
// Styled-components for the grid and card elements
...
// Wrapper to position the labels in the top right corner of the image
...
// Styling for the individual labels (for sale, for rent)
